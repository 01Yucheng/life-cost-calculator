...
class GitHubStorage:
    def __init__(self):
        try:
            ...
            st.error("⚠️ GitHub Secrets 配置错误，请检查 Token 和 Repo 名称。")
            st.stop()
...
with st.expander("➕ 录入新房源", expanded=True):
    ...
    with c1:
        ...
        loc_in = st.text_input("📍 车站名", placeholder="例如：新大久保駅")
    ...
    if st.button("🚀 AI 分析并保存", use_container_width=True):
        if loc_in:
            with st.spinner("AI 正在计算通勤开销..."):
                ...
                # 此处省略 get_transit 调用逻辑，可添加您的 AI prompt 逻辑
                new_row = pd.DataFrame([{
                    ...
                    "月房租(円)": rent_in,
                    "管理费(円)": 5000,
                    "学费(单程)": 200, "塾费(单程)": 300, "线路概要": "AI 自动生成的线路..."
                }])
...
if not edited_df.equals(st.session_state.df_houses):
    ...
    storage.save_data(edited_df) # 实时改动同步到 GitHub
...
if not st.session_state.df_houses.empty:
    ...
    # CSS：优化 PDF 打印分页
    st.markdown('<style>@media print {.stContainer {page-break-inside: avoid; border: 1px solid #ddd !important; margin-bottom: 10px;}}</style>', unsafe_allow_html=True)
        ...
        try:
            ...
            with st.container(border=True):
                ...
                with t_col:
                    ...
                    st.write(f"🏠 房租+管理: {int(float(row['月房租(円)'])+float(row['管理费(円)'])):,} | 🚇 月通勤: {int(fare_m):,}")
                with b_col:
                    m_api = "https://www.google.com/maps/dir/?api=1"
                    url_s = f"{m_api}&origin={urllib.parse.quote(row['房源位置'])}&destination={urllib.parse.quote(dest_school)}&travelmode=transit"
